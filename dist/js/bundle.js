...
new _vue2.default({
  ...
  methods: {
    add_todo: function add_todo() {
      ...
      if (isNaN(this.del_idBox[0].id) === false) {
        //isNaN は数字でないとき「true」になる。よって数字の時は「false」となる。
    ...
    list_delete: function list_delete(del) {
      ...
      //this.del_id = this.todo_list[del_index].id; //配列削除前に、削除予定idを、別建てに変数化しておく。
      this.del_idBox.push({
        id: this.todo_list[del_index].id //配列削除前に、削除予定idを、別建てに配列変数化しておく。
      });
      this.todo_list.splice(del_index, 1); //削除
    ...
    kensaku_list_delete: function kensaku_list_delete(del) {
      console.log(del);
      // this.del_id = this.kensaku_todo_list[del].id; //配列削除前に、削除予定idを、別建てに変数化しておく。
      this.del_idBox.push({
        id: this.kensaku_todo_list[del].id //配列削除前に、削除予定idを、別建てに配列変数化しておく。
      });
    ...
    kensaku_li_style: function kensaku_li_style(itemId) {
      if (!this.todo_list[itemId].checkbox) {
        this.kensaku_todo_list[itemId].backgroundColor = '#C0C0C0';
        this.kensaku_todo_list[itemId].completed_date = '(完了)' + Date();
  ...
  computed: {
    ...
    // 文字検索（絞り込み機能）
    kensaku: function kensaku() {
      var result = [];
      for (var i in this.kensaku_todo_list) {
        var hoge = this.kensaku_todo_list[i];
        if (hoge.todo.indexOf(this.kensaku_value) !== -1) {
          // 文字が見つからない場合は「-1」が返る。ここでは、ここで文字が見つかった場合、配列resultに文字を挿入することになる。
          result.push(hoge);
        }
      }
      return result;
